...
import com.mindgeeks.sportsnews.adapters.adapterBallingScoreCard
import com.mindgeeks.sportsnews.adapters.adapterBattingScoreCard
...
class Fragment_ScoreCard : Fragment() {
    ...
    override fun onCreateView(
        ...
        try {
            if (bundle != null) {
                val tempData = bundle.getParcelable<score_Card>("scoreData")
                if (tempData is score_Card) {
                    data = tempData
                    ...
                    setHeading()
                    binding.teamABalling.rv.adapter = adapterBallingScoreCard(data.teamsScoreCard.teama.bowlers)
                    binding.teamBBalling.rv.adapter = adapterBallingScoreCard(data.teamsScoreCard.teamb.bowlers)
                    binding.teamABatting.rv.adapter = adapterBattingScoreCard(data.teamsScoreCard.teama.batsmen)
                    binding.teamBBatting.rv.adapter = adapterBattingScoreCard(data.teamsScoreCard.teamb.batsmen)
